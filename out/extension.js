...
function activate(context) {
    ...
    let openInBrowser = vscode.commands.registerCommand('openmd.openBrowser', async (uri) => {
    ...
    let openInPreview = vscode.commands.registerCommand('openmd.openPreview', async (uri) => {
        const fileUri = uri || vscode.window.activeTextEditor?.document.uri;
        if (!fileUri) {
            vscode.window.showErrorMessage('No markdown file selected');
            return;
        ...
        try {
            ...
            if (currentPanel) {
                ...
                // สร้าง panel ใหม่
                currentPanel = vscode.window.createWebviewPanel('markdownPreview', `Preview: ${fileName}`, vscode.ViewColumn.Two, {
                    enableScripts: true,
...
async function markdownToHtml(markdown, fileUri) {
...
async function markdownToWebviewHtml(markdown, fileUri, panel) {
    const body = await (0, marked_1.marked)(markdown);
    const fileName = path.basename(fileUri.fsPath, '.md');
    return getWebviewTemplate(body, fileName);
}
// Template สำหรับ Browser
function getHtmlTemplate(body, title) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getWebviewTemplate(body, title) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
